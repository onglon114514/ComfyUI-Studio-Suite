...
- 低显存训练打标基线：WD14 原始 tag + Gemma 4 E4B Q4 纯文本补自然语言 caption
- 可选视觉任务：`Gemma 4 E4B Vision GGUF` + 对应 `mmproj`
...
- WD14 负责输出原始 tag。
- Task Agent 保持 WD14 tag 第一段完全不改。
- `Gemma 4 E4B Q4` 只负责补第二段自然语言 caption。
- 后端用 `llama_cpp_python_inproc`，context `4096`，GPU layers `0`，batch `128`。
- 批量队列期间保持 LLM 加载，队列结束后再用清理节点卸载。
...
这需要视觉 GGUF 和匹配 `mmproj`，不作为低显存默认基线。
...
- `examples/workflows/tagging_wd14_llm_anima_train_preview.json`
  - WD14 tag + 本地 LLM 的自然语言 caption 辅助
  - 面向训练打标工作流
  - 低显存基线使用纯文本 Gemma 4 E4B Q4；视觉修正是可选扩展
...
- `prompt_studio/storage/`
  - 随包提供的 Prompt Studio autocomplete、分组 tag、本地补全 tag 和提示词数据
- `docs/`
  - 安装、后端、迁移、发布说明
- `scripts/`
  - 打包、自检、辅助脚本
